...
fn process_records(reader: Box<dyn BufRead>, writer: &mut Box<dyn Write>) -> io::Result<()> {
    ...
    let mut records: std::collections::HashMap<(String, String), (f64, usize)> = std::collections::HashMap::new();
        ...
        match serde_json::from_str::<InputRecord>(&line) {
            Ok(record) => {
                if let Some(avg_rate) = calculate_average_rate(&record) {
                    let key = (record.name, record.billing_code);
                    let entry = records.entry(key).or_insert((0.0, 0));
                    entry.0 += avg_rate;
                    entry.1 += 1;
    ...
    print_processing_summary(&stats, records.len());

    for ((name, billing_code), (total, count)) in records {
        let avg_rate = total / count as f64;
        if avg_rate <= 30.0 {
...
fn print_processing_summary(stats: &ProcessingStats, unique_records: usize) {
    println!("\nProcessing Summary:");
    println!("Total lines processed: {}", stats.total_lines);
    println!("Parsing errors: {}", stats.error_count);
    println!("Successfully processed records: {}", stats.successful_records);
    println!("Unique records found: {}", unique_records);
}
